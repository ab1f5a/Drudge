...
import com.ab1f5a.drudge.util.*;

@DontSaveState
public final class TunnelBorer extends Feature
	implements UpdateListener, RenderListener
{
	...
	private final SliderSetting limit = new SliderSetting("Limit",
		"description.drudge.setting.tunnelborer.limit", 0, 0, 1000, 1,
		ValueDisplay.INTEGER.withSuffixKey("gui.drudge.unit.blocks")
			.withLabelKey(1, "gui.drudge.unit.one_block")
			.withLabelKey(0, "gui.drudge.unit.disabled"));

	private final CheckboxSetting torches = new CheckboxSetting("Place torches",
		"description.drudge.setting.tunnelborer.place_torches", false);
	...
	public Component getRenderName()
	{
		...
		return Component.translatable(
			"gui.drudge.feature.tunnelborer.progress", length,
			limit.getValueI());
	}
	...
	{
		DRUDGE.getModules().areaExcavator.setEnabled(false);
	...
	private void showActionBar()
	{
		...
		MC.gui.setOverlayMessage(
			Component.translatable("gui.drudge.hud.tunnelborer"), false);
	}

	@Override
	public void onUpdate()
	{
		DRUDGE.getModules().autoToolSwitch.setEnabled(false);
		showActionBar();
	...
	public void onRender(PoseStack matrixStack, float partialTicks)
	{
		...
		for(EasyVertexBuffer buffer : vertexBuffers)
		{
			...
			buffer.draw(matrixStack, DrudgeRenderLayers.ESP_LINES);
		}
	...
	private class DigTunnelTask extends Task
	{
		...
		public void run()
		{
			...
			if(!boxes.isEmpty())
				vertexBuffers[1] =
					EasyVertexBuffer.createAndUpload(VertexFormat.Mode.LINES,
			...
			if(currentBlock == null)
			{
				...
				{
					ChatUtils.component(
						Component.translatable("chat.drudge.tunnelborer.completed"));
					setEnabled(false);
				}
			...
			DRUDGE.getModules().autoToolSwitch.equipBestTool(currentBlock, false, true,
				0);
	...
	private class WalkForwardTask extends Task
	{
		...
		public void run()
		{
			...
			DRUDGE.getRotationFaker().faceVectorClientIgnorePitch(vec);
	...
	private class FillInFloorTask extends Task
	{
		...
		public void run()
		{
			...
			{
				ChatUtils.error(Component
					.translatable("chat.drudge.tunnelborer.floor_hole"));
				setEnabled(false);
				return;
			}
	...
	private class DodgeLiquidTask extends Task
	{
		...
		public boolean canRun()
		{
			...
			ChatUtils.error(
			Component.translatable("chat.drudge.tunnelborer.flooded"));
		...
		public void run()
		{
			...
			DRUDGE.getRotationFaker().faceVectorClientIgnorePitch(toVec3d(pos3));
	...
	private class PlaceTorchTask extends Task
	{
		...
		public void run()
		{
			...
			{
				ChatUtils.error(
					Component.translatable("chat.drudge.tunnelborer.out_of_torches"));
				setEnabled(false);
				return;
			}
	...
	private void placeBlockSimple(BlockPos pos)
	{
		...
		DRUDGE.getRotationFaker().faceVectorPacket(hitVec);
	...
	private boolean breakBlock(BlockPos pos)
	{
		...
		DRUDGE.getRotationFaker().faceVectorPacket(hitVecs[side.ordinal()]);
