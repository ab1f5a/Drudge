...
{
	/**
	 * 26.x carries the line width as a vertex attribute and this project used to
	 * write 2.0 into it. Here it lives in the DynamicTransforms uniform instead,
	 * so the same value is supplied per draw call.
	 */
	...
	private EasyVertexBuffer(MeshData buffer, VertexFormat.Mode drawMode)
	{
	...
	private EasyVertexBuffer(VertexFormat.Mode drawMode)
	{
		...
		float green, float blue, float alpha)
	{
		...
		GpuBufferSlice gpuBufferSlice = RenderSystem.getDynamicUniforms()
			.writeTransform(new Matrix4f(RenderSystem.getModelViewMatrix()),
				new Vector4f(red, green, blue, alpha), new Vector3f(),
				RenderSystem.getTextureMatrix(), LINE_WIDTH);
		...
		try(RenderPass renderPass =
			RenderSystem.getDevice().createCommandEncoder().createRenderPass(
				() -> "something from Drudge", framebuffer.getColorTextureView(),
				...
				OptionalDouble.empty()))
		{
			...
			renderPass.drawIndexed(indexCount, 1, 0, 0);
		}
